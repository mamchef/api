...
class PaymentCalculationService
{
    ...
    public static function calculatePaymentSplit(
        ...
        $finalChefFoodAmount = $originalChefAmount - $discountSplit['chef_deduction'];
    ...
    private static function calculateDiscountSplit(float $discountAmount, float $appFee, float $sharePercentage): array
    {
        ...
        return match($strategy) {
            'app_fee_first' => self::appFeeFirstStrategy($discountAmount, $appFee),
            'chef_first' => self::chefFirstStrategy($discountAmount, $appFee),
            'dynamic' => self::dynamicStrategy($discountAmount),
            default => self::appFeeFirstStrategy($discountAmount, $appFee)
        };
    }
     ...
     */
    private static function appFeeFirstStrategy(float $discountAmount, float $appFee): array
    {
        if ($discountAmount <= $appFee) {
            // Discount is less than or equal to app fee - deduct all from app fee
            return [
                'app_fee_deduction' => $discountAmount,
                'chef_deduction' => 0.0
            ];
        } else {
            // Discount exceeds app fee - deduct all app fee, rest from chef
            return [
                'app_fee_deduction' => $appFee,
                'chef_deduction' => $discountAmount - $appFee
            ];
        }
    }
     ...
     */
    private static function chefFirstStrategy(float $discountAmount, float $appFee): array
    {
        // For this strategy, we need to know chef's share, but we'll apply it proportionally
        $appFeePercentage = config('app.discount_app_fee_percentage', 0);
        $chefPercentage = config('app.discount_chef_percentage', 100);
        
        $chefDeduction = ($discountAmount * $chefPercentage) / 100;
        $appFeeDeduction = ($discountAmount * $appFeePercentage) / 100;
        
        // Make sure we don't exceed available app fee
        if ($appFeeDeduction > $appFee) {
            $excessDeduction = $appFeeDeduction - $appFee;
            $appFeeDeduction = $appFee;
            $chefDeduction += $excessDeduction;
        }
        
        return [
            'app_fee_deduction' => $appFeeDeduction,
            'chef_deduction' => $chefDeduction
        ];
    }
